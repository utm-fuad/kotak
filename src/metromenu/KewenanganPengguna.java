...
import Model.ModelKewenanganPengguna;
import metromini.*;
 ...
 * @author asus
 */
public class KewenanganPengguna extends javax.swing.JPanel {
    ...
    private JTextField kodegroup;  
    private koneksi.KompletTextFieldCombo kompletTextFieldCombo2;
    Model.ModelKewenanganPengguna model;
    private int kodeuserpriviledge, idlogaudit;
    private String kueri;
    
    /**
     * Creates new form PanelHalaman1
     */
    public KewenanganPengguna() {    
        initComponents();
    }
    
    public KewenanganPengguna(HomePage _homePage, String _namaTabel) {
        this();
        this.homePage = _homePage;
        this.namaTabel = _namaTabel;
        model = new ModelKewenanganPengguna();
    ...
    private void initPKAutoIncrement() {
        query = "select \"Kode_user_priviledge\" " +
                " from pdpt_dev.\"TRAN_USER_PRIVILEDGE\" " +
                " order by \"Kode_user_priviledge\" desc";
        kon.selectData(query);     
        String [][] data = kon.dapatkanData();
        if (data[0][0] == null) {
            kodeuserpriviledge = 1;
        } else {
            kodeuserpriviledge = Integer.parseInt(data[0][0]);
            kodeuserpriviledge++;
        }        
        System.out.println("kodeuserpriviledge: " + kodeuserpriviledge);
    }
    ...
    private void initAutoComplete() {
        ...
        query = "select \"Username\", \"Kode_perguruan_tinggi\" from pdpt_dev.\"TMST_USERNAME\""; 
        kompletTextFieldCombo1.initDataUtkAutoComplete(query);        
        username = kompletTextFieldCombo1.dapatkanTextField();
        usernameField = kompletTextFieldCombo1.dapatkanComboBox();        
        
        kompletTextFieldCombo2 = new KompletTextFieldCombo(kon, 
                kodegroupField, kodegroup);        
        query = "select a.\"Kode_group_user_priviledge\", b.\"Nama_role\" " +
                " from pdpt_dev.\"TRAN_GROUP_USER_PRIVILEDGE\" a, \"TREF_ROLE_USER\" b " +
                " where a.\"Kode_role_user\"=b.\"Kode_role_user\""; 
        kompletTextFieldCombo2.initDataUtkAutoComplete(query);        
        kodegroup = kompletTextFieldCombo2.dapatkanTextField();
        kodegroupField = kompletTextFieldCombo2.dapatkanComboBox();        
    }
    
    private void inisialisasiData() {
        namaTabel = "pdpt_dev.\"Tran_user_priviledge\"";
    ...
    public KewenanganPengguna(String [][] data, String [] kolom) {
        this();
        tampilkanDataKeTabel(data, kolom);
    }
    ...
    private void persiapanEntriDataBaru() {
        initPKAutoIncrement();
        kodeuserField.setText("" + kodeuserpriviledge);
        username.setText("");
        kodegroup.setText("");
    ...
    private void tampilkanData(int row) {
        ...
        kodeuserField.setText(tabel.getValueAt(row, 0).toString());
        username.setText(tabel.getValueAt(row, 1).toString());        
        kodegroup.setText(tabel.getValueAt(row, 2).toString());
        
    }
    
    public String [] dapatkanNilaiUntukInsert() {       
        String [] data = new String[jmlKolom];
        data[0] = (!kodeuserField.getText().equals("")?kodeuserField.getText():"0");
        
        String datausernamementah = username.getText();
        System.out.println("data username mentah: " + datausernamementah);
        String [] datausername = datausernamementah.split(" ");
        System.out.println("data username yg dipilih: " + datausername[0]);
        data[1] = datausername[0];
        
        String datakodegroupmentah = kodegroup.getText();
        System.out.println("data kodegroup mentah: " + datakodegroupmentah);
        String [] datakodegroup = datakodegroupmentah.split(" ");
        System.out.println("data kodegroup yg dipilih: " + datakodegroup[0]);
        data[2] = datakodegroup[0];
    ...
    public String[] dapatkanNilaiUntukUpdate(){
        ...
        String [] datausername = datausernamementah.split(" ");
        System.out.println("data username yg dipilih: " + datausername[0]);
        data[0] = datausername[0];
        
        String datakodegroupmentah = kodegroup.getText();
        System.out.println("data kodegroup mentah: " + datakodegroupmentah);
        String [] datakodegroup = datakodegroupmentah.split(" ");
        System.out.println("data kodegroup yg dipilih: " + datakodegroup[0]);
        data[1] = datakodegroup[0];
        
        data[2] = "" + idlogaudit;
        data[3] = (!kodeuserField.getText().equals("")?kodeuserField.getText():"0");
        return data;
    }
    
    public String[] dapatkanNilaiUntukDelete(){
        String [] data = new String[1];
        data[0] = kodeuserField.getText();
        return data;   
    }
    ...
    private void initComponents() {
        ...
        kodeuserField = new javax.swing.JFormattedTextField();
        usernameField = new javax.swing.JComboBox();
        kodegroupField = new javax.swing.JComboBox();
        ...
        kodetenagaahliLabel.setText("Kode User Priviledge:");
        panelLabel.add(kodetenagaahliLabel);

        namatenagaahliLabel.setText("Username :");
        panelLabel.add(namatenagaahliLabel);

        posisiLabel.setText("Kode Group Menu:");
        ...
        kodeuserField.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat(""))));
        panelTextField.add(kodeuserField);

        panelTextField.add(usernameField);

        panelTextField.add(kodegroupField);
...
//            dapatkanNilaiUntukUpdate() );
    ...
    private javax.swing.JComboBox kodegroupField;
    private javax.swing.JLabel kodetenagaahliLabel;
    private javax.swing.JFormattedTextField kodeuserField;
